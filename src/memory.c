...
static LoogalIdentity *find_identity_by_sha(LoogalMemory *m, const char *sha256) {
    ...
    for (size_t i = 0; i < m->identity_count; i++) {
        if (strcmp(m->identities[i].sha256, sha256) == 0) {
...
static LoogalLocation *find_location_by_path(LoogalMemory *m, const char *path) {
    ...
    for (size_t i = 0; i < m->location_count; i++) {
        if (strcmp(m->locations[i].path, path) == 0) {
...
static uint64_t next_identity_id(const LoogalMemory *m) {
    ...
    for (size_t i = 0; i < m->identity_count; i++) {
        if (m->identities[i].id > max) max = m->identities[i].id;
    }
...
static uint64_t next_location_id(const LoogalMemory *m) {
    ...
    for (size_t i = 0; i < m->location_count; i++) {
        if (m->locations[i].id > max) max = m->locations[i].id;
    }
...
static void recompute_location_counts(LoogalMemory *m) {
    for (size_t i = 0; i < m->identity_count; i++) {
        m->identities[i].location_count = 0;
    }
...
int loogal_memory_load(LoogalMemory *m) {
    ...
    if (f) {
        ...
        while (fgets(line, sizeof(line), f)) {
            ...
            extract_u64_field(line, "mtime_unix", &loc.mtime_unix);
...
static int write_identities_tmp(const LoogalMemory *m, const char *tmp) {
    ...
    for (size_t i = 0; i < m->identity_count; i++) {
        ...
        fprintf(f, "{\"id\":%llu,\"type\":\"visual.identity\",\"sha256\":\"%s\",\"dhash\":%llu,\"ahash\":%llu,\"aspect\":%.6f,\"best_file_size\":%llu,\"width\":%d,\"height\":%d,\"location_count\":%llu,\"first_seen_unix\":%llu,\"last_seen_unix\":%llu}\n",
                ...
                id->aspect,
                (unsigned long long)id->best_file_size,
                id->width,
                id->height,
...
static int write_locations_tmp(const LoogalMemory *m, const char *tmp) {
    ...
    for (size_t i = 0; i < m->location_count; i++) {
        ...
        fprintf(f, "{\"id\":%llu,\"type\":\"visual.location\",\"identity_id\":%llu,\"path\":\"%s\",\"kind\":\"%s\",\"file_size\":%llu,\"mtime_unix\":%llu,\"seen_unix\":%llu,\"status\":%u}\n",
                (unsigned long long)loc->id,
                (unsigned long long)loc->identity_id,
                p ? p : "",
                loc->kind,
                (unsigned long long)loc->file_size,
                (unsigned long long)loc->mtime_unix,
...
int loogal_memory_mark_known_location_if_unchanged(LoogalMemory *m, const char *path, uint64_t file_size, uint64_t mtime_unix, int touch) {
    ...
    if (touch) {
        ...
        loogal_memory_append_event("location.seen.fast", "ok", path, loc->identity_id, "known unchanged location skipped before image probe");
    }
...
int loogal_memory_ingest_image(LoogalMemory *m, const LoogalImageInfo *info) {
    ...
    if (existing_path) {
        if (existing_path->identity_id == identity->id) {
            ...
            existing_path->mtime_unix = info->mtime_unix;
    ...
    loc->mtime_unix = info->mtime_unix;
    loc->seen_unix = ts;
...
int loogal_memory_build_records(const LoogalMemory *m, LoogalRecord **out_records, size_t *out_count) {
    ...
    *out_count = 0;
    ...
    size_t active = 0;
    for (size_t i = 0; i < m->location_count; i++) {
        if (m->locations[i].status == LOOGAL_LOCATION_ACTIVE) active++;
    }
    ...
    for (size_t i = 0; i < m->location_count; i++) {
        ...
        for (size_t j = 0; j < m->identity_count; j++) {
            if (m->identities[j].id == loc->identity_id) {
                id = &m->identities[j];
                break;
            }
        }
